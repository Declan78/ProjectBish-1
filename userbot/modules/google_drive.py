...
async def google_drive(gdrive):
    if not gdrive.pattern_match.group(1) and not gdrive.reply_to_msg_id:
        return
    file_name = gdrive.pattern_match.group(1).strip()
    ...
    downloadURL = await upload(gdrive, file_name, mimeType)
    await gdrive.edit(
        "`[FILE - UPLOAD]`"
        f" • `Name  :` `{file_name}`\n"
        " • `Status :` "
        f"Sucessfully uploaded to: [Google Drive]({downloadURL}"
    )
    return


async def upload(gdrive, file_name, mimeType):
    service = await create_app(gdrive)
